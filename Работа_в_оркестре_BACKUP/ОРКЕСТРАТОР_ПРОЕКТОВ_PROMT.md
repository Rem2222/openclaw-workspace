...
**Этап 0: При запуске оркестратора** — прочитать `АКТИВНЫЕ_ПРОЕКТЫ.md`:
...
### 2.1. ОБНОВЛЕНИЕ РЕЕСТРА ПРОЕКТОВ

**После каждого завершённого этапа** обновить `АКТИВНЫЕ_ПРОЕКТЫ.md`:
- Изменить статус этапа
- Если проект завершён → перенести в таблицу "Завершённые" (зачеркнуть)
- Если новый проект → добавить в таблицу "Текущие"

**Формат обновления:**
| Проект | Статус | Этап | Папка | Дата | Модель | Время |
|--------|--------|------|-------|------|--------|-------|

**Обязательно записывать:**
- **Модель** — какая модель использовалась для этого этапа (ollama/qwen3.5:397b, glm-5 и т.п.)
- **Время** — сколько реального времени ушло на этап (можно примерно: "30 мин", "2 часа")
...
├── АКТИВНЫЕ_ПРОЕКТЫ.md              # 📋 Реестр всех проектов (ЧИТАТЬ ПРИ СТАРТЕ!)
...
### Этап 0: Читать реестр проектов (ОБЯЗАТЕЛЬНО!)
**При каждом запуске оркестратора:**

1. Прочитать `АКТИВНЫЕ_ПРОЕКТЫ.md`
2. Определить:
...
cd ~/.openclaw/workspace/projects/НазваниеПроекта
```

**После создания SPEC.md** — добавить проект в `АКТИВНЫЕ_ПРОЕКТЫ.md`:
- Название проекта
- Статус: 🆕 НОВЫЙ
- Этап: Аналитика требований
- Папка: путь к проекту
- Дата начала: сегодня

**После создания проекта** — добавить метку `coded by romul` в README.md:
```
coded by romul
# Название проекта
```
Если README.md не существует — создать его с меткой.
...
### После завершения этапа — создать задачу в Beads:
```bash
bd create "[Этап] Краткое описание | File: путь/к/файлу.md"
```

### Список задач проекта:
```bash
bd list --project <имя>   # показать задачи проекта
bd ready                   # показать доступные задачи
bd show <id>              # детали задачи
```

### Статусы:
- open → in_progress (--claim) → closed
...
**При запуске:**
1. Читаю `АКТИВНЫЕ_ПРОЕКТЫ.md`
2. Докладываю сводку проектов
3. Проверяю `bd ready` — есть ли задачи в очереди
