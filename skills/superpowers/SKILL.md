...
### При завершении subagent — отправить краткий бриф Роману

Когда subagent завершается, я получаю результат. После этого — отправить Роману КРАТКИЙ бриф:

```
✅ [Название задачи] — [1-2 предложения что сделано]
```

**Пример:**
```
✅ bd:workspace-7ki — исправлена кнопка Сессия/Агент. Теперь работает.
```

**НЕ нужно:**
- Пересылать полный отчёт subagent
