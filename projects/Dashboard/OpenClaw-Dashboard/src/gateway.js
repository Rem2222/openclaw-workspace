...
class GatewayClient {
  ...
  async getTasks(filters = {}) {
    try {
      ...
      console.log('[GatewayClient.getTasks] Response received');
      console.log('[GatewayClient.getTasks] Response.data keys:', Object.keys(sessionsResponse.data || {}));
      ...
      if (sessionsResponse.data?.result?.content?.[0]?.text) {
        console.log('[GatewayClient.getTasks] Parsing from content[0].text');
        try {
          const parsed = JSON.parse(sessionsResponse.data.result.content[0].text);
          sessions = parsed.sessions || [];
          console.log('[GatewayClient.getTasks] Parsed sessions count:', sessions.length);
        } catch (e) {
      ...
      // Проверяем details.sessions формат
        ...
        console.log('[GatewayClient.getTasks] Using details.sessions');
      ...
      // Проверяем sessions формат
        ...
        console.log('[GatewayClient.getTasks] Using result.sessions');
      ...
      console.log('[GatewayClient.getTasks] Tasks created:', tasks.length);
      return tasks;
    } catch (error) {
