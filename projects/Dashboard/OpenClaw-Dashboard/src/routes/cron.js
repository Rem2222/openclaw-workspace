...
router.get('/', async (req, res) => {
  try {
    ...
    console.error('❌ Error in /api/cron:', error.message);
...
router.get('/:id', async (req, res) => {
  try {
    ...
    console.error('❌ Error in /api/cron/:id:', error.message);
    res.status(500).json({ error: error.message });
  }
});
...
router.post('/', async (req, res) => {
  try {
    ...
    console.error('❌ Error in POST /api/cron:', error.message);
    res.status(500).json({ error: error.message });
  }
});
...
router.put('/:id', async (req, res) => {
  try {
    ...
    console.error('❌ Error in PUT /api/cron/:id:', error.message);
    res.status(500).json({ error: error.message });
  }
});
...
router.delete('/:id', async (req, res) => {
  try {
    ...
    console.error('❌ Error in DELETE /api/cron/:id:', error.message);
    res.status(500).json({ error: error.message });
  }
});
...
router.post('/:id/trigger', async (req, res) => {
  try {
    ...
    console.error('❌ Error in POST /api/cron/:id/trigger:', error.message);
    res.status(500).json({ error: error.message });
  }
});
