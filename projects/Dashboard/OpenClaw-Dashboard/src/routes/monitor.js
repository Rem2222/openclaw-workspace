...
function parseSessionActivity(sessionKey, limit = 100, offset = 0) {
  ...
  try {
    ...
    console.error('[monitor route] Error reading sessions.json:', e.message);
  }
  ...
  try {
    ...
    console.error('[monitor route] Error parsing session:', error.message);
    return { items: [], total: 0 };
  }
}
...
router.get('/session-activity', async (req, res) => {
  ...
  try {
    ...
    console.error('[monitor route] Error:', error.message);
