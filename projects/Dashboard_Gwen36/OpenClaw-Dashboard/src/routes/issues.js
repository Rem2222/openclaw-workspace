...
router.get('/', async (req, res) => {
  try {
    ...
    if (issuesCache.data && 
      ...
      if (process.env.NODE_ENV === 'development') console.log(`[Issues API] Cache HIT for key "${cacheKey}", age: ${now - issuesCache.timestamp}ms`);
    ...
    if (process.env.NODE_ENV === 'development') console.log(`[Issues API] Cache MISS for key "${cacheKey}"`);
    ...
    issuesCache.params = cacheKey;
    if (process.env.NODE_ENV === 'development') console.log(`[Issues API] Cache UPDATED for key "${cacheKey}"`);
    ...
    if (process.env.NODE_ENV === 'development') console.error('[Issues API] Error:', error.message);
    res.status(500).json({ error: error.message });
  }
});

// GET /api/issues/session-task-map — получить все привязки session -> task (defined before /:id to avoid route conflict)
...
function normalizeIssue(raw) {
  ...
  if (process.env.NODE_ENV === 'development') console.log('[DEBUG] extractProject:', { title: raw.title, filePath: raw.file_path, project });
