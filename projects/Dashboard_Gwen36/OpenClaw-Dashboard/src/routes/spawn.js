...
7. После планирования — спроси: какой агент использовать для разработки?`;
    ...
    if (process.env.NODE_ENV === 'development') console.log('[Spawn] Launching Superpowers session:', projectName);
    ...
    if (process.env.NODE_ENV === 'development') console.log('[Spawn] Result:', JSON.stringify(result.data).slice(0, 200));
    
    res.json({ ok: true, label, projectName });
  } catch (error) {
    if (process.env.NODE_ENV === 'development') console.error('[Spawn] Error:', error.message);
