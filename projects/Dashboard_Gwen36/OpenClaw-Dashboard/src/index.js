...
// DEBUG: Log env vars
if (isDev) console.log('[DEBUG] CWD:', process.cwd());
if (isDev) console.log('[DEBUG] GATEWAY_URL:', process.env.GATEWAY_URL);
if (isDev) console.log('[DEBUG] GATEWAY_TOKEN length:', process.env.GATEWAY_TOKEN?.length);
if (isDev) console.log('[DEBUG] PORT:', process.env.PORT);
...
const httpServer = app.listen(PORT, () => {
  if (isDev) console.log(`🚀 OpenClaw Dashboard running on http://localhost:${PORT}`);
});
...
if (isDev) console.log(`🔄 Real-time polling started (interval: ${POLL_INTERVAL}ms)`);
