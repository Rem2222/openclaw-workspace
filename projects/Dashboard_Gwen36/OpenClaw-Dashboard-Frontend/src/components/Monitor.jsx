...
export default function Monitor() {
  ...
  // Автовыбор сессии когда filteredSessions обновился после смены проекта
  useEffect(() => {
    if (projectChangedRef.current && filteredSessions.length > 0 && !selectedSession && projectFilter) {
      const active = filteredSessions.find(s => s.status !== 'done') || filteredSessions[0];
  ...
  const currentProjectTaskIds = useMemo(() => 
    currentProject?.issues.map(i => i.id) || [],
    [currentProject]
  );

  const currentProjectSessionKeys = useMemo(() => 
    Object.entries(taskSessionMap)
      .filter(([_, issueId]) => currentProjectTaskIds.includes(issueId))
      .map(([sessionKey]) => sessionKey),
    [taskSessionMap, currentProjectTaskIds]
  );

  // Task progress: "done/total"
  const taskProgressText = useMemo(() => {
    if (!currentProject) return '0/0';
    const done = currentProject.issues.filter(i => i.status === 'closed').length;
    const total = currentProject.issues.length;
    return `${done}/${total}`;
  }, [currentProject]);

  // Task progress bar JSX
  const taskProgressBar = useMemo(() => {
    if (!currentProject || currentProject.issues.length === 0) return null;
    const done = currentProject.issues.filter(i => i.status === 'closed').length;
    const total = currentProject.issues.length;
  ...
  // Time tracker text
  const timeTrackerText = useMemo(() => {
    if (!currentProject || currentProject.issues.length === 0) return '—';
    
    const projectSessions = allSessions
      .filter(s => currentProjectSessionKeys.includes(s.key) && s.endedAt && s.startedAt);
    ...
    const firstTaskCreated = currentProject.issues.reduce((earliest, issue) => {
  ...
  }, [currentProject, currentProjectSessionKeys, allSessions]);

  // Subagent count
  const subagentCount = useMemo(() => {
    const subagentSessions = allSessions.filter(s => 
      (s.key?.includes('subagent') || s.label?.startsWith('bd:') || s.label?.includes('subagent')) &&
      currentProjectSessionKeys.includes(s.key)
    );
    return subagentSessions.length;
  }, [allSessions, currentProjectSessionKeys]);

  // Subagent average time text
  // Subagent list for the subagent section
  const subagentListContent = useMemo(() => {
    const isSubagent = (s) => 
  ...
  }, [allSessions, projectFilter, projects, taskSessionMap]);


  const subagentAvgText = useMemo(() => {
    const projectTaskIds = currentProject?.issues.map(i => i.id) || [];
    const projectSessionKeys = Object.entries(taskSessionMap)
      .filter(([_, issueId]) => projectTaskIds.includes(issueId))
      .map(([sessionKey]) => sessionKey);
    const subagentSessions = allSessions.filter(s => 
  ...
  return (
    <div className="page" style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      {/* Project Stats Header — always rendered with placeholder when no project */}
      <div className="project-stats-header" style={{ minHeight: '120px' }}>
        {projectFilter ? (
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', flex: 1, minHeight: 0, maxHeight: 'calc(100vh - 280px)' }}>
        {/* Left: Activity Feed */}
        <div className="card" style={{ display: 'flex', flexDirection: 'column', flex: collapsedSections.activity ? '0 0 auto' : 1, minHeight: collapsedSections.activity ? 'auto' : '150px', maxHeight: '100%', overflow: 'hidden' }}>
          <h3 
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', minHeight: 0 }}>
          {/* Tasks */}
          <div className="card" style={{ display: 'flex', flexDirection: 'column', flex: collapsedSections.tasks ? '0 0 auto' : 1, minHeight: collapsedSections.tasks ? 'auto' : '150px', overflow: 'hidden' }}>
            <h3 
            ...
            >
              ...
              {projects.length === 0 ? (
                ...
                (() => {
                  // Group ALL issues by type first
                  const allIssues = projects.flatMap(p => p.issues.map(i => ({ ...i, _project: p.name })));
                  const byType = {};
                  ...
                  return TYPE_ORDER.map(type => {
                    ...
                    // Group by project within this type
                    const byProject = {};
                    typeIssues.forEach(issue => {
                      const proj = issue._project || 'Без проекта';
                    ...
                    return (
                      <div key={type} style={{ marginBottom: '16px' }}>
                        ...
                        {Object.entries(byProject).map(([projName, projIssues]) => (
                          <div key={projName} style={{ marginBottom: '10px', paddingLeft: '8px' }}>
