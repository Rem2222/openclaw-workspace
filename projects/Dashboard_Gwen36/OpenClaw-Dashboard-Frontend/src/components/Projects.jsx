...
export default function Projects() {
  ...
  const displayItems = useMemo(() => {
    ...
    // Group: type → project → issues
    const items = [];
    const byType = {};
    sorted.forEach(issue => {
      const type = getIssueType(issue.title);
      const proj = issue.project || 'Без проекта';
      if (!byType[type]) byType[type] = {};
      if (!byType[type][proj]) byType[type][proj] = [];
      byType[type][proj].push(issue);
    });
    
    TYPE_ORDER.forEach(type => {
      const projects = byType[type];
      if (!projects) return;
      items.push({ type: 'type-header', data: type, key: 'type-' + type });
      
      Object.entries(projects).forEach(([projName, projIssues]) => {
        items.push({ type: 'project-header', data: { project: projName, count: projIssues.length }, key: 'proj-' + type + '-' + projName });
    ...
    <div className="page">
      <div className="page-header">
        <div className="header-left">
          <h1>Проекты</h1>
          <span className="badge badge-info">{projectCountBadge}</span>
        </div>
              ...
              <tbody>
                {displayItems.map(item => {
                  if (item.type === 'type-header') {
                    return (
                      <tr key={item.key} style={{ background: 'var(--surface)', borderTop: '2px solid var(--border)' }}>
                        <td colSpan={6} style={{ padding: '10px 12px', fontWeight: 600, fontSize: '13px', color: TYPE_COLORS[item.data] || 'var(--text-muted)' }}>
                          {item.data}
                        </td>
                      </tr>
                    );
                  ...
                  if (item.type === 'project-header') {
                    return (
                      <tr key={item.key} style={{ background: 'var(--surface)', opacity: 0.8 }}>
                        <td colSpan={6} style={{ padding: '6px 12px', fontWeight: 500, fontSize: '12px', color: 'var(--accent)' }}>
                          📁 {item.data.project} ({item.data.count})
                        </td>
                      </tr>
                    );
                  }
                        ...
                        <td colSpan={7}>
                          <div className="detail-grid">
                            ...
                            <div className="detail-item" style={{ gridColumn: '1 / -1', marginTop: '8px', display: 'flex', gap: '8px' }}>
                              ...
                              {issue.id && Object.values(sessionTaskMap).includes(issue.id) && sessionLink(issue)}
