...
```bash
python3 linear_api.py attachment-create <issue_id> "Название файла" "<url>" "Описание"
```
...
3. **Прикрепить файлы** (если агент создал файлы):
   ```bash
   python3 linear_api.py attachment-create <issue_id> "SPEC.md" "/path/to/SPEC.md" "Спецификация задачи"
   ```
