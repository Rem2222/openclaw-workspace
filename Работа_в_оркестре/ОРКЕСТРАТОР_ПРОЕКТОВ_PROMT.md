...
**Также в Этап 0:** Если есть активный проект — запросить контекст из ByteRover:
```bash
brv query "контекст проекта <название проекта>"
brv swarm query "архитектурные решения <название проекта>"
```
...
### ByteRover — База знаний проекта

**ByteRover** хранит паттерны, решения и архитектурные правила в `.brv/context-tree/`.

**Перед началом работы** — запросить контекст проекта:
```bash
brv query "<вопрос о проекте>"
# или для поиска без LLM (быстрее, бесплатно):
brv search "<вопрос>"
```

**После завершения этапа** — сохранить ключевые решения:
```bash
brv curate "<описание решения/паттерна>"
```

**Команды ByteRover:**
| Команда | Назначение |
|---------|-----------|
| `brv query "вопрос"` | Запрос с синтезом через LLM |
| `brv search "запрос"` | BM25 поиск (без LLM, быстро) |
| `brv swarm query "вопрос"` | Поиск по всем провайдерам памяти |
| `brv status` | Статус базы |

**Важно:** ByteRover использует MiniMax API (настроен по умолчанию).
